<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="评审专家" prop="pingshenzhuanjia">
          <el-input v-model="ruleForm.pingshenzhuanjia" 
              placeholder="评审专家" clearable  :readonly="ro.pingshenzhuanjia"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="评审专家" prop="pingshenzhuanjia">
              <el-input v-model="ruleForm.pingshenzhuanjia" 
                placeholder="评审专家" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="评审日期" prop="pingshenriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.pingshenriqi" 
                type="date"
                placeholder="评审日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.pingshenriqi" label="评审日期" prop="pingshenriqi">
              <el-input v-model="ruleForm.pingshenriqi" 
                placeholder="评审日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="项目描述" prop="xiangmumiaoshu">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="项目描述"
                  v-model="ruleForm.xiangmumiaoshu" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xiangmumiaoshu" label="项目描述" prop="xiangmumiaoshu">
                    <span>{{ruleForm.xiangmumiaoshu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="学生团队" prop="xueshengtuandui">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="学生团队"
                  v-model="ruleForm.xueshengtuandui" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xueshengtuandui" label="学生团队" prop="xueshengtuandui">
                    <span>{{ruleForm.xueshengtuandui}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="成员介绍" prop="chengyuanjieshao">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="成员介绍"
                  v-model="ruleForm.chengyuanjieshao" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.chengyuanjieshao" label="成员介绍" prop="chengyuanjieshao">
                    <span>{{ruleForm.chengyuanjieshao}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="评委意见" prop="pingweiyijian">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="评委意见"
                  v-model="ruleForm.pingweiyijian" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.pingweiyijian" label="评委意见" prop="pingweiyijian">
                    <span>{{ruleForm.pingweiyijian}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='pingshenzhuanjia'){
            this.ruleForm.pingshenzhuanjia = obj[o];
	    this.ro.pingshenzhuanjia = true;
            continue;
          }
          if(o=='pingweiyijian'){
            this.ruleForm.pingweiyijian = obj[o];
	    this.ro.pingweiyijian = true;
            continue;
          }
          if(o=='pingshenriqi'){
            this.ruleForm.pingshenriqi = obj[o];
	    this.ro.pingshenriqi = true;
            continue;
          }
        }
      }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhuanjiapingshen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhuanjiapingshenCrossAddOrUpdateFlag = false;
                  this.parent.search();
